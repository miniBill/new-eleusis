...
jLoop2 : Int -> Bool -> HS -> Blocks -> HS
jLoop2 j first hs blocks =
    let
        ( ab, h, d ) =
            if first then
                let
...
outerLoop : Bool -> HS -> Int -> Int -> Int -> Int -> Message -> Int -> HS
outerLoop first hs block start bytes index message length =
    let
        blocks =
            makeBlocks block
        ...
        hs2 =
            jLoop2 0 first hs blocks5

        hs3 =
            sumHS hs hs2
    in
    if not end then
        outerLoop False hs3 block2 start2 bytes2 index3 message length
...
hash string =
    let
        ...
        hs2 =
            outerLoop True hs block start bytes index message length
    in
    toHex64 hs2
